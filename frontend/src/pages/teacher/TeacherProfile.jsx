...
export default function TeacherProfile() {
  ...
  const details = user?.teacherDetails || {};
        ...
        <div className="px-8 pb-8">
            ...
            <div className="flex items-end space-x-5">
              <div className="h-24 w-24 rounded-full border-4 border-white bg-white shadow-lg overflow-hidden flex items-center justify-center bg-gradient-to-tr from-indigo-100 to-purple-100">
                {details.photoUrl ? (
                  ...
                  <span className="text-3xl font-bold text-indigo-600">{user?.name?.charAt(0) || 'T'}</span>
                )}
              </div>
              <div className="pb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <InfoItem 
              ...
              value={details.phone || details.contactNumber} 
            />
            <InfoItem 
              icon={Mail} 
              label="Email Address" 
              value={user?.email || details.email} 
            />
          </div>
