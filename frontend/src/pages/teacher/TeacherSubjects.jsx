...
import { Calendar as CalendarIcon, Clock, BookOpen, FileText, CheckCircle, Edit3 } from 'lucide-react';
...
export default function TeacherSubjects() {
  ...
  const [viewMode, setViewMode] = useState('syllabus'); // Default to syllabus as per request
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Class Syllabus & Schedule</h1>
          <p className="text-sm text-gray-500 mt-1">View and manage syllabus and exam date sheets for your assigned classes.</p>
        </div>

        {/* Class Selector */}
        <div className="bg-white border border-gray-200 rounded-xl px-4 py-2 shadow-sm flex items-center">
          <span className="text-sm font-medium text-gray-500 mr-3">Select Class:</span>
          <select 
            value={selectedClass}
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {currentData.syllabus.map((item) => (
                <div key={item.id} className="bg-white border border-gray-100 rounded-xl p-5 shadow-sm hover:shadow-md transition-all relative group">
                  <div className="flex justify-between items-start mb-4 pb-3 border-b border-gray-100">
                    ...
                    <button className="text-gray-400 hover:text-indigo-600 p-1 rounded-md hover:bg-indigo-50 transition-colors opacity-0 group-hover:opacity-100">
